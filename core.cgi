...
_HTML_
	...
	$IN{'cmd'}=~s/(?:^|;)(?:d|z|re)new(?:=[^;]*)?(?:;|$)//go; #new系はcmdとして保存されると困る

=item コマンドで使えるもの

icon    : 専用アイコン
iconlist: (nolist|economy)
absoluteIcon : 絶対指定アイコン
relativeIcon : 相対指定アイコン
...
lockThread:	スレッドをロックします（管理パスワードが必要）
...
=cut
	...
	if($EX{'lockThread'}&&$IN{'i'}){
		...
		unless($CF{'admps'}&&$IN{'pass'}eq$CF{'admps'}
			...
			my%DT=$log[0]=~/([^\t]*)=\t([^\t]*);\t/go;
			die 'あなたはこのスレッドをロックできません。'unless&mircrypt($DT{'time'},$IN{'pass'},$DT{'pass'});
		}
		...
		$IN{'page'}=1;
		$IN{'rvs'}='';
		&showRvsMenu(sprintf"第$IN{'i'}番スレッドのロック%s成功",$isLocking?'':'解除');
	}
...
=cut
		...
		if($CF{'admps'}&&$IN{'oldps'}eq$CF{'admps'}){
			...
			index($log[$IN{'j'}],"Mir12=\tLocked")+1&&&showUserError("第$IN{'i'}番の$IN{'j'}はロックされている");
			$log[$#log]=~/Mir12=\tLocked[^\t]*\brevise\b/o&&&showUserError('このスレッドは固くロックされている');
			#Pass変更
			$IN{'oldps'}=$IN{'pass'};
		}
...
=cut
	...
	else{print qq(<H2 class="mode">Something Wicked happend!(modeが不明)</H2>).&getFooter;exit;}
	#処理成功-Indexに戻る
	if($_[0]){
		print<<"_HTML_";
<DIV class="center">
...
_HTML_
			...
			index($log[$IN{'j'}],"Mir12=\tLocked")+1&&&showRvsMenu("第$IN{'i'}番の$IN{'j'}はロックされています。");
			$log[$#log]=~/Mir12=\tLocked[^\t]*\bdelete\b/o&&&showRvsMenu('このスレッドは固くロックされています。');
