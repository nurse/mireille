...
=cut
	...
	index($zero[0],"Mir12=\t")<0&&die"ZEROのログ形式がMir12型以外です";
	%Z0=($zero[0]=~/([^\t]*)=\t([^\t]*);\t/go);
...
=cut
		...
		$log[$IN{'j'}]=~/Mir12=\t(?:del|Locked)/&&&showUserError("第$IN{'i'}番の$IN{'j'}は修正できない");
		my%DT=($log[$IN{'j'}]=~/([^\t]*)=\t([^\t]*);\t/go);
...
sub delArticle{
	...
	seek(RW,0,0);
	my@log=<RW>;
	$log[$IN{'j'}]=~/Mir12=\t(?:del|Locked)/&&&showRvsMenu("第$IN{'i'}番の$IN{'j'}は削除できません。");
	my%DT=($log[$IN{'j'}]=~/([^\t]*)=\t([^\t]*);\t/go);
	#削除分岐
	SWITCH:{
		if($CF{'admps'}&&$IN{'pass'}eq$CF{'admps'}){
